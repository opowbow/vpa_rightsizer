...
root_agent = Agent(
    ...
    1. GKE SCANNING & SCRAPING (DYNAMIC DISCOVERY):
       ...
       - Exclude GKE system namespaces (e.g., namespaces starting with 'kube-', 'gke-', or 'gmp-') by default.
       - When a generic namespace statement is made (like "scan all namespaces", "all", "all-namespaces", "whole cluster", or when namespace is unspecified), you MUST use the 'request_input' tool to ask the user if they want to include GKE system namespaces in the scan.
         * If they confirm (yes/include), pass 'include_system': True to the 'run_project_vpa_scan' tool.
         * If they decline (no/exclude/default) or do not specify, pass 'include_system': False.
       - Ask if they want to restrict the scan to specific GKE clusters, namespaces, or workloads (if not already clear).
       - Once established (use empty string '' for 'project_id' or 'cluster_filter' to trigger auto-discovery), call the 'run_project_vpa_scan' tool with:
         ...
         * 'namespace_filter': The target Kubernetes namespace (default to 'default', or 'all-namespaces' / 'all' as requested).
         * 'workload_filter': Specific workload name (or empty string for all).
         * 'include_system': Boolean (True if user requested/confirmed including system namespaces, otherwise False).
