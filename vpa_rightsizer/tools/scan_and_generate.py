...
def main():
    ...
    parser.add_argument(
        "--include-system",
        action="store_true",
        help="Include system namespaces in the scan",
    )
    ...
    if args.clusters:
        ...
        try:
                        ...
                        target_clusters.append(
                            {
                                "name": name,
                                "location": c.get(
                                    "location", c.get("zone", "europe-north1")
                                ),
            ...
            print(
                f"Warning: Failed to query cluster list: {e}. Defaulting location to europe-north1."
            )
            for name in names:
                target_clusters.append(
                    {"name": name, "location": "europe-north1", "status": "RUNNING"}
                )
    else:
    ...
    for cluster_info in target_clusters:
        ...
        try:
            is_all_namespaces = not args.namespace or args.namespace.lower() in (
                "all",
                "all-namespaces",
            )
            namespace_arg = (
                ["--all-namespaces"] if is_all_namespaces else ["-n", args.namespace]
            )
            deps_json = run_cmd(
                ["kubectl", "get", "deployments", "-o", "json", *namespace_arg]
            )
            ...
            if not getattr(args, "include_system", False) and (
                is_all_namespaces or not args.namespace
            ):
        ...
        if args.workload:
            deployments = [
                d for d in deployments if d["metadata"]["name"] == args.workload
            ]
        ...
        try:
            namespace_arg = (
                ["-n", args.namespace] if args.namespace else ["--all-namespaces"]
            )
        ...
        manifest_target_dir = os.path.join(
            results_dir,
            f"vpa-{project_id}",
            f"vpa-{cluster_name}",
            args.namespace or "all-namespaces",
        )
        ...
        for dep in deployments:
            ...
            if (
                "template" in clean_dep["spec"]
                and "metadata" in clean_dep["spec"]["template"]
            ):
            ...
            vpa_mode = "Off"
            vpa_enabled = (
                "No (Cloud Monitoring Fallback)"
                if not has_vpa_api
                else "No (VPA API Available)"
            )
            if vpa:
                vpa_mode = (
                    vpa.get("spec", {}).get("updatePolicy", {}).get("updateMode", "Off")
                )
            ...
            for c in containers:
                ...
                if vpa:
                    ...
                    if rec_container:
                        ...
                        rec_mem = rec_container.get("target", {}).get(
                            "memory", curr_mem
                        )
                        vpa_lower_cpu = rec_container.get("lowerBound", {}).get(
                            "cpu", rec_cpu
                        )
                        vpa_lower_mem = rec_container.get("lowerBound", {}).get(
                            "memory", rec_mem
                        )
                        vpa_upper_cpu = rec_container.get("upperBound", {}).get(
                            "cpu", rec_cpu
                        )
                        vpa_upper_mem = rec_container.get("upperBound", {}).get(
                            "memory", rec_mem
                        )
                        rec_provided = True
    ...
    with open(report_file, "w", encoding="utf-8") as rf:
        rf.write(
            "# 📊 GKE Workloads & Vertical Pod Autoscaler (VPA) Recommendations Report\n\n"
        )
        rf.write(
        ...
        rf.write(
            f"- **Cloud Monitoring Fallbacks**: {len(all_workloads) - vpa_enabled_count}\n\n"
        )
        ...
        for cluster_info in target_clusters:
            ...
            rf.write(
                f"- Cluster `{cName}` contains {len(c_workloads)} workloads scanned.\n"
            )
        rf.write("\n")

        rf.write("## 📋 Detailed Workload Recommendation Table\n")
        rf.write(
            "| Cluster Name | Namespace | Workload Name | VPA Status & Mode | Container Name | Current Requests | Recommended Targets | Lower Bounds | Upper Bounds | Manifest Path |\n"
        )
        rf.write("|---|---|---|---|---|---|---|---|---|---|\n")
        for w in all_workloads:
            rf.write(
                f"| {w['cluster']} | {w['namespace']} | {w['workloadName']} | {w['vpaStatus']} | {w['containerName']} | {w['current']} | {w['recommended']} | {w['lowerBound']} | {w['upperBound']} | {w['manifestPath']} |\n"
            )
