...
def deploy_dashboard_to_gke(cluster_name: str, project_id: str = "") -> str:
    ...
    try:
        ...
        region_sub = f"{loc_parts[0]}-{loc_parts[1]}" if len(loc_parts) >= 2 else "europe-north1"
        create_repo_cmd = [
        ...
        subprocess.run(
            ["kubectl", "rollout", "status", "deployment/vpa-web-report", "-n", "default", "--timeout=120s"],
            capture_output=True, text=True, check=True
        )
        ...
        for attempt in range(18):
            ip_cmd = ["kubectl", "get", "service", "vpa-web-report-service", "-n", "default", "-o", "jsonpath={.status.loadBalancer.ingress[0].ip}"]
...
def deploy_dashboard_to_cloud_run(project_id: str = "", region: str = "europe-west1") -> str:
    """
    ...
    try:
        ...
        deploy_cmd = [
            "gcloud", "run", "deploy", "vpa-web-report",
            "--image", image_tag,
            "--region", region,
            "--platform", "managed",
